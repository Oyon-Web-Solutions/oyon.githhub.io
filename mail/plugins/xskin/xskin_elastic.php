...
class xskin_elastic extends xskin_larry
{
    protected function elasticInitialize()
    {
        ...
        // include styles
        $this->include_stylesheet('https://fonts.googleapis.com/css?family=Cairo&subset=latin-ext');
        $this->include_stylesheet('https://fonts.googleapis.com/css?family=Montserrat+Alternates&subset=latin-ext');
        $this->include_stylesheet('https://fonts.googleapis.com/css?family=Sarala&subset=latin-ext');
        $this->include_stylesheet('https://fonts.googleapis.com/css?family=Quattrocento&subset=latin-ext');
        $this->include_stylesheet('https://fonts.googleapis.com/css?family=Merienda&subset=latin-ext');
    ...
    public function elasticStartup()
    {
        ...
        // set the preview background logo (loaded using js in [skin]/watermark.html)
        // first take the value from the skin config; if it doesn't exist, try the xskin config;
        ...
        $this->rcmail->output->set_env("rcp_skin", $this->isRcpSkin($this->getSkin()));
    ...
    protected function elasticModifyLoginHtml(&$arg)
    {
        ...
        // and if it doesn't exist, try the xskin config
        $skin = $this->getSkin();
        $logo = $this->rcmail->config->get(
            "xlogo_login",
            $this->rcmail->config->get("login_branding_$skin")
        );
    ...
    private function includeSkinConfig()
    {
        ...
        $file = RCUBE_INSTALL_PATH . "skins/" . $this->getSkin() . "/config.inc.php";
    ...
    private function getCurrentFontFamily()
    {
        ...
        return $this->rcmail->config->get(
            "xskin_font_family_" . $this->getSkin(),
    ...
    private function getCurrentThickFont()
    {
        ...
        return $this->rcmail->config->get(
            "xskin_thick_font_" . $this->getSkin(),
    ...
    private function getCurrentListIcons()
    {
        ...
        return $this->rcmail->config->get(
            "xskin_list_icons_" . $this->getSkin(),
    ...
    private function getCurrentButtonIcons()
    {
        ...
        return $this->rcmail->config->get(
            "xskin_button_icons_" . $this->getSkin(),
