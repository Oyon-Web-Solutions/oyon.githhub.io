...
class xskin_larry extends XFramework\Plugin
{
    ...
    public function larryStartup()
    {
        ...
        $configLogo = $this->rcmail->config->get("skin_logo", array());

        if (is_array($configLogo)) {
            $configLogo["*"] =
                $this->rcmail->config->get("header_branding_$skin", "skins/$skin/assets/images/logo_header.png");
            $configLogo["messageprint"] = $configLogo["contactprint"] =
                $this->rcmail->config->get("print_branding_$skin", "skins/$skin/assets/images/logo_print.png");
        }
    ...
    /**
     * Returns the current skin. On the login/logout pages on RC 1.4 it returns the skin set in the config; on RC 1.3
     * the default_skin property doesn't exist, so it returns the user-selected skin.
     *
     * @return mixed
     */
    protected function getSkin()
    {
        ...
        return $this->rcmail->config->get("skin", "larry");
    }
}